...
import { getSliderValueFor } from '../../store/reducer';
...
const BitcoinCheck = () => {
  ...
  const slidervalue = useSelector(state => getSliderValueFor(state));
  ...
  useEffect(() => {
    axios.get( process.env.REACT_APP_API_HOST+'api/getBitcoinValue/'+ slidervalue )
        .then((response) =>{
          ...
          <div className="bitcoin-check-wrapper">
            <div className="bitcoin-left-warp">
              <div className="bitcoin-img">
                <img src={BitcoinCheckoutImg} alt="bitcoin check" />
              </div>
              ...
              <div className="bitcoin-address-details">
                ...
                <span>{walletAddress}
                  <span onClick={() => {navigator.clipboard.writeText(walletAddress)}} className="copy-address">
                    <svg width="16" height="16" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" clip-rule="evenodd" d="M30 0C31.1 0 32 0.9 32 2V22C32 23.1 31.1 24 30 24H24V30C24 31.1 23.1 32 22 32H2C0.9 32 0 31.1 0 30V10C0 8.9 0.9 8 2 8H8V2C8 0.9 8.9 0 10 0H30ZM20 12H4V28H20V12ZM28 4H12V8H22C23.1 8 24 8.9 24 10V20H28V4Z" fill="#36352C"></path></svg>
                  </span>
                </span>
              </div>
              <div className="bitcoin-address-details">
                <h5>STEP 1: COPY YOUR WALLET ADDRESS</h5>
                <span>Complete your transfer of {BtcPrice}BTC to address</span>
              </div>
              <div className="bitcoin-address-details">
                <h5>TRANSACTIONS NEDDS TO CONFIRMED BY THE BITCOIN NETWORK</h5>
              </div>
